...
imagepaths = list(paths.list_images(args["images"]))  #imutilsin içindeki list_images fonksiyonunu çağırdık
...
for imagePath in imagepaths: # resim dosya yollarının içinde dönen bir döngü oluşturduk
    image = cv2.imread(imagepaths)   # görselleri yükledik ve görsel listesini güncelledik
    images.append(image)
